...
word_pins = {
    "DIT_IS": Pin(8, Pin.OUT),
    "VYF_M": Pin(7, Pin.OUT),    # First "FIVE" GPIO
    "TIEN_M": Pin(17, Pin.OUT),  # First "TEN" GPIO
    "KWART": Pin(18, Pin.OUT),
    "TWINTIG": Pin(20, Pin.OUT),
    "HALF": Pin(16, Pin.OUT),
    "OOR": Pin(14, Pin.OUT),
    "VOOR": Pin(13, Pin.OUT),
    "EEN": Pin(6, Pin.OUT),
    ...
    "DRIE": Pin(12, Pin.OUT),
    "VIER": Pin(3, Pin.OUT),
    "VYF_H": Pin(11, Pin.OUT),  # "FIVE" for hours aka The Lower FIVE
    "SES": Pin(9, Pin.OUT),
    "SEWE": Pin(10, Pin.OUT),
    "AGT": Pin(4, Pin.OUT),
    "NEGE": Pin(2, Pin.OUT),
    "TIEN_H": Pin(1, Pin.OUT),  # Second "TEN" GPIO aka The Lower TEN
    "ELF": Pin(15, Pin.OUT),
    "TWAALF": Pin(21, Pin.OUT),
    "MINUTE": Pin(19, Pin.OUT),
    "UUR": Pin(0, Pin.OUT),
}
...
def update_word_clock(ds, wifiFlag, dsFlag):
    ...
    word_pins["DIT_IS"].value(1)

    # Determine the words to light up based on the rounded minutes
    if rounded_minute == 0:
        word_pins["UUR"].value(1)
    elif minute >= 55:
        word_pins["UUR"].value(1)
        hour += 1
    elif rounded_minute <= 30:
        # "PAST" logic
        word_pins["OOR"].value(1)
        if rounded_minute == 5:
            word_pins["VYF_M"].value(1)
        elif rounded_minute == 10:
            word_pins["TIEN_M"].value(1)
        elif rounded_minute == 15:
            word_pins["KWART"].value(1)
        elif rounded_minute == 20:
            word_pins["TWINTIG"].value(1)
    ...
    hour_words = [
        "EEN", "TWEE", "DRIE", "VIER", "VYF_H", "SES",
        "SEWE", "AGT", "NEGE", "TIEN_H", "ELF", "TWAALF"
    ]
